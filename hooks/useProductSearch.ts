...
interface UseProductSearchResult {
  ...
  search: (query: string, categoryUrl?: string) => void;
  loadMore: () => void;
  reset: () => void;
}
...
export function useProductSearch(
  ...
  const [categoryUrl, setCategoryUrl] = useState<string | undefined>();
  ...
  const fetchProducts = useCallback(async (
    ...
    try {
      ...
      if (!result) {
        result = await fetchFromScraper(searchQuery, pageNum, catUrl, signal);
        dataSource = 'scraper';
      }
  ...
  useEffect(() => {
    const debounceTimer = setTimeout(() => {
      fetchProducts(query, 1, categoryUrl);
    }, query ? 300 : 0); // No debounce for initial load

    return () => {
      clearTimeout(debounceTimer);
    };
  }, [query, categoryUrl, fetchProducts]);

  // Search function to trigger new search
  const search = useCallback((newQuery: string, catUrl?: string) => {
    setQuery(newQuery);
  ...
  const loadMore = useCallback(() => {
    if (!isLoading && !isLoadingMore && hasMore) {
      fetchProducts(query, page + 1, categoryUrl, true);
    }
  }, [isLoading, isLoadingMore, hasMore, query, page, categoryUrl, fetchProducts]);

  // Reset to initial state
  const reset = useCallback(() => {
    setQuery('');
    setCategoryUrl(undefined);
    setPage(1);
    setProducts([]);
    setHasMore(false);
    fetchProducts('', 1);
